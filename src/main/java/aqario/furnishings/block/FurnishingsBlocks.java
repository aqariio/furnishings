...
import aqario.furnishings.item.IronScaffoldingItem;
...
public class FurnishingsBlocks {
    ...
    public static final Block BRAZIER = register("brazier", new BrazierBlock(true, 1, QuiltBlockSettings.of(Material.METAL, MapColor.IRON_GRAY).strength(3.0f).sounds(BlockSoundGroup.METAL).luminance((state) -> state.get(BrazierBlock.LIT) ? 15 : 0).nonOpaque()), ItemGroup.DECORATIONS);
    public static final Block SOUL_BRAZIER = register("soul_brazier", new BrazierBlock(false, 2, QuiltBlockSettings.of(Material.METAL, MapColor.IRON_GRAY).strength(3.0f).sounds(BlockSoundGroup.METAL).luminance((state) -> state.get(BrazierBlock.LIT) ? 10 : 0).nonOpaque()), ItemGroup.DECORATIONS);
    ...
    private static Item registerBlockItem(String id, Block block, ItemGroup group) {
        return Registry.register(Registry.ITEM, new Identifier(Furnishings.MODID, id), new BlockItem(block, new QuiltItemSettings().group(group)));
    }

    private static Item registerScaffoldingBlockItem(String id, Block block) {
        return Registry.register(Registry.ITEM, new Identifier(Furnishings.MODID, id), new IronScaffoldingItem(block, new QuiltItemSettings().group(ItemGroup.DECORATIONS)));
    }
